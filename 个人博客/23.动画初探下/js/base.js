...
Base.prototype.animate = function (obj) {
    for (var i = 0; i < this.elements.length; i ++) {
        ...
        var start = obj['start'] != undefined ? obj['start'] :  parseInt(getStyle(element, attr));	//可选，默认是CSS的起始位置
        ...
        timer = setInterval(function () {
            if (type == 'buffer') {
                step = (target -  parseFloat(getStyle(element, attr))) / speed;
                step = step > 0 ? Math.ceil(step) : Math.floor(step);
            }
            console.log(Math.abs(parseFloat(getStyle(element,attr))))
            if (step == 0) {
                setTarget();
            } else if (step > 0 && Math.abs( parseFloat(getStyle(element, attr)) <= step)) {
                setTarget();
            } else if (step < 0 &&  parseFloat(getStyle(element, attr)) <= Math.abs(step)) {
                setTarget();
            } else {
