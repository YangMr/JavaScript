...
Base.prototype.animate = function (obj) {
    for (var i = 0; i < this.elements.length; i ++) {
        ...
        var start = obj['start'] != undefined ? obj['start'] :  parseFloat(getStyle(element, attr));	//可选，默认是CSS的起始位置
        ...
        timer = setInterval(function () {
            ...
            if (step > 0 &&  parseFloat(getStyle(element, attr)) >= target) {
                element.style[attr] = target + 'px';
                clearInterval(timer);
            } else if (step < 0 &&  parseFloat(getStyle(element, attr)) <= target) {
                element.style[attr] = target + 'px';
                clearInterval(timer);
            }
        }, t);
